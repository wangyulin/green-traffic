...
@Service
@ConditionalOnProperty(name = "metrics.sink", havingValue = "influx", matchIfMissing = true)
public class InfluxMetricAdapter implements MetricWritePort {
    private static final Logger log = LoggerFactory.getLogger(InfluxMetricAdapter.class);

    private final MetricsProperties props;
    private final RestTemplate rest;

    public InfluxMetricAdapter(MetricsProperties props) {
        this.props = props;
    ...
    public void start() {
        scheduler = Executors.newSingleThreadScheduledExecutor(r -> {
            Thread t = new Thread(r, "influx-metrics-flusher");
            t.setDaemon(true);
            return t;
        });
        ...
        try {
            ...
            while (attempts <= props.getMaxRetries()) {
                try {
                    ...
                    ResponseEntity<String> resp = rest.postForEntity(props.getInfluxUrl(), entity, String.class);
                    log.debug("[InfluxAdapter] write response: {}", resp.getStatusCode());
                    return;
                } catch (Exception ex) {
                    attempts++;
                    log.warn("[InfluxAdapter] write attempt {} failed, will retry after {} ms", attempts, delay, ex);
                    try { Thread.sleep(delay); } catch (InterruptedException ie) { Thread.currentThread().interrupt(); break; }
                    delay = delay * 2;
                }
            }
            log.error("[InfluxAdapter] exhausted retries writing {} points", drained.size());
        } catch (Exception ex) {
            log.error("[InfluxAdapter] unexpected flush error", ex);
        }
    }
