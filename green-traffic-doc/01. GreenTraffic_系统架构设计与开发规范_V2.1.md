...
2. `Message`、`MessageSubscriber`、`TrafficMessageTypes` 等架构契约目前位于 `common`，Port 所有权不够清晰；
3. `green-traffic-model` 与实际领域模型源码位置存在不一致，例如 `TrafficMetric` 实际位于 common 源码目录；
4. `green-traffic-simulator` 直接依赖 Core，这本身可以接受，但需要明确它属于“输入适配器 / 数据生产适配层”，而不是领域核心；
5. `publishAsync()` 已设计，但当前 `CarbonEmissionSimulator`、`SumoTrafficSimulator` 调用的都是 `publish()`；
6. Dev 的 Spring Events 当前 `publish()` 是同步事件分发，并非真正异步；
7. VM 环境的 RocketMQ Publisher 虽然有 `publishAsync()`，但当前仿真器没有使用它；
8. Consumer 从消息到 UseCase 的业务调用也是同步执行的，只有 MQ 客户端本身负责消息投递；
9. VictoriaMetrics 写入端已有批量队列和重试，但它是“性能缓冲”，不能替代 RocketMQ 的可靠消息能力；
10. 当前 `RocketMQMessageSubscriber` 存在 payload 反序列化适配逻辑，但消息 Schema 版本、幂等、DLQ、Trace 等生产能力还需要补齐。
...
但 `TrafficMetric` 实际源码路径位于 common 下，这应该整改。
