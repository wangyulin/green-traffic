...
> 迁移状态（Messaging）：`MessagePublisher` / `MessageSubscriber` 已在 `green-traffic-core` 定义，对应适配器 `RocketMQMessagePublisher` / `RocketMQMessageSubscriber` 与 `SpringEventsMessagePublisher` / `SpringEventsMessageSubscriber` 已实现于 `green-traffic-infrastructure`，装配通过 `green-traffic-bootstrap`。剩余项：幂等、DLQ/重试、契约测试与运行配置文档仍需补充，参见 `MIGRATION_MESSAGING.md`。
