...
- 核心用例（端口→应用服务）：`WriteTrafficMetricUseCase` 的实现 `MetricApplicationService.write(...)` 将 `WriteTrafficMetricCommand` 转为 ~~MetricPoint~~（已迁移为域对象 `TrafficMetric`），并调用输出能力端口 `TrafficMetricStore`（原 `MetricWritePort`）。参见 `MetricApplicationService` — `MetricApplicationService.java`

- 写入时序库（Adapter）：输出能力端口 `TrafficMetricStore`（原 `MetricWritePort`）在 vm 配置下由 `VictoriaMetricAdapter` 实现，它把 ~~MetricPoint~~（已迁移为域对象 `TrafficMetric`）批量转换为 Influx line-protocol（或 VM 支持的格式），并通过 HTTP 写入 VictoriaMetrics（有异步批量/重试逻辑）。参见 `VictoriaMetricAdapter.write`（及 flush 实现） — `VictoriaMetricAdapter.java`
